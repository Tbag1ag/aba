...
class StorageService {
  ...
  async importData(json: string): Promise<void> {
    ...
    if (this.sql) {
      ...
      // Build a map from old category id -> new category name (for quotes to reference)
      // Import categories - ignore old IDs, let SERIAL auto-assign new ones
      ...
      // Import quotes - ignore old IDs, let SERIAL auto-assign new ones
      for (const q of data.quotes) {
        await this.sql`
          ...
          VALUES (
            ...
            ${q.is_pinned || false},
            ${q.confidence || 0.7},
            ${q.last_accessed_at || q.created_at || new Date().toISOString()},
            ${q.created_at || new Date().toISOString()}
          )
        `;
      }

      // Reset sequences to avoid future ID conflicts
      await this.sql`SELECT setval('categories_id_seq', (SELECT MAX(id) FROM categories))`;
      await this.sql`SELECT setval('quotes_id_seq', (SELECT MAX(id) FROM quotes))`;
